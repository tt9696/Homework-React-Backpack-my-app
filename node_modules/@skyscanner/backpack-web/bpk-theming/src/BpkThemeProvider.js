...
const createStyle = (theme, themeAttributes) => {
  ...
  flattenedThemeAttributes.forEach(attribute => {
    if (theme[attribute]) {
      const cssName = attribute.replace(/([A-Z])/g, variable => `-${variable.toLowerCase()}`).replace(/([0-9])/, variable => `-${variable.toLowerCase()}`);
...
const themeAttributesPropType = (props, propName, componentName) => {
  ...
  const extraneousThemeAttributes = {
    ...theme
  };
  const missingThemeAttributes = [];
  ...
  const errors = [];
  if (missingThemeAttributes.length > 0) {
    errors.push(`${componentName}: To apply theming, the theme prop must include \`${themeAttributes.join(', ')}\` (missing \`${missingThemeAttributes.join(', ')}\`)`);
  }
  if (Object.keys(extraneousThemeAttributes).length > 0) {
    errors.push(`${componentName}: Extraneous theme attributes supplied: \`${Object.keys(extraneousThemeAttributes).join(', ')}\`.`);
  }
  if (errors.length > 0) {
